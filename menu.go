...
func (menu *Menu) UpdateMenu(req MenuCreate) (TblMenus, error) {
	...
	menudet.SeparateWindow = req.SeperateWindow
...
func (menu *Menu) GetDirectSubMenusByParentID(parentid int, tenantid string) ([]TblMenus, error) {
	...
	Menulist, err := menumodel.GetDirectSubMenusByParentID(parentid, tenantid, menu.DB)
...
//Update Order index for MEnu

func (menu *Menu) UpdateMenuOrderIndexes(Orderindex int, menuid, parentid int, userid int, tenantid string) (bool, error) {
	...
	menudet.OrderIndex = Orderindex

	menudet.TenantId = tenantid

	err := menumodel.UpdateMenuOrderIndex(&menudet, menuid, parentid, menu.DB, tenantid)
...
func (menu *Menu) GetMenuBySlugName(slug string, websiteid int, tenantid string) (TblMenus, error) {
	...
	GetData, err := menumodel.GetMenuBySlugName(slug, websiteid, menu.DB, tenantid)
	if err != nil {
		return TblMenus{}, err
	}
